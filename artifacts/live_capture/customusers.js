...
BeforeLoad(function () {
	...
	if('type' in queryString && ValidateUtil.isNumber(queryString.type))
		$type = parseInt(queryString.type);

	$("#BREADCRUMB").attr("nivel", "menu_usertype" + $type);
});
...
LoadPage(function(){
	...
	if($type === undefined)
		return;
	var temp_u = new window['usertype' + $type]
	is_visitor = temp_u.functions.is_visitor();
    //initUsersPage();
	...
	var tbl = new Table($('#tbl_custom_user'), window['usertype' + $type],
		{
			'modal' :{
				'open' : function(modal){
					var deletedTemplates = false;
					var deletedFaceTemplates = false;
					...
					if (is_visitor) {
						$("#template_devices div.span12 div.control-group:last").hide();
					}
					...
					modal.getContent().find("#delDigitais").click(function () {
						if (!deletedTemplates) {
							lstLoadedTemlates.forEach(function(item){
								item.add = false;
								item.isNew = true;
							});
							totalCount -= lstLoadedTemlates.length;
							refreshCount();
				...
				'save' : function(modal){
					...
					modal.getObject().templates = lstLoadedTemlates.concat(lst);
			...
			'callbackRemove': function(lstRemove) {
				...
				window['usertype' + $type].fn.sendJson(dataImageRemove);
				window['usertype' + $type].fn.sendJson(dataUserRemove);
			}
		}
	);
	tbl.load();
...
function CadastrarDigital(duress) {
	...
	socket.onerror = function(error) {
		...
		var msg = {
			error: "Error while connecting to service",
			info: "",
		}
	...
	var enrollInterval = setInterval(function(){
		...
		if(enroller.enroller_state == "NORMAL_STATE") {
			if(enroller.last_enroll) {
				...
				$('#logFacial').html("Face enrolled successfully.");
				refreshCountFace();
				refreshUserFace();
			} else if(enroller.last_enroll_error == "UNKNOWN"){
				clearInterval(enrollInterval);
				$('#logFacial').html("Face enrollment cancelled.");
			}
		...
		if(enroller.enroller_state == "ENROLL_FACE_STATE"){
			$('#logFacial').html("Precessing enrollment");
			return;
		}
